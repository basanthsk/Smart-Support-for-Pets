...
const validatePassword = (password: string) => {
  ...
  // The dot (.) permits special characters like '@'.
...
const Login: React.FC = () => {
  ...
  const formatFirebaseError = (err: any) => {
    ...
    switch (code) {
      ...
      default: return err.message || "An authentication error occurred.";
    }
  };
  ...
  const validate = () => {
    if (!isLogin) {
      ...
      if (!validateEmail(formData.identifier)) { setError("Invalid email address."); return false; }
      ...
      if (!agreedToTerms) { setError("You must agree to the terms."); return false; }
    }
    return true;
  };
      ...
      <div className="max-w-md w-full bg-white rounded-[3rem] shadow-2xl border border-slate-100 p-8 md:p-12 animate-in fade-in zoom-in-95 duration-700">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          {!isLogin && (
            <div className="space-y-1">
              ...
              <input name="fullName" type="text" placeholder="John Doe" value={formData.fullName} onChange={handleChange} className="w-full bg-slate-50 border border-slate-100 rounded-2xl py-4 px-6 text-sm font-bold text-slate-900 focus:bg-white outline-none transition-all" />
            </div>
          )}
          <div className="space-y-1">
            ...
            <input name="identifier" type="text" placeholder="Enter details" value={formData.identifier} onChange={handleChange} className="w-full bg-slate-50 border border-slate-100 rounded-2xl py-4 px-6 text-sm font-bold text-slate-900 focus:bg-white outline-none transition-all" />
          </div>
          {!isLogin && (
            <div className="space-y-1">
        ...
        <div className="mt-8 text-center border-t border-slate-50 pt-6">
          <p className="text-slate-400 text-[11px] font-black uppercase tracking-widest">
            {isLogin ? "New to Paw Pal?" : "Already a member?"}
